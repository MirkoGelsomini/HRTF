...
function jsonLoad(name) {
  jsonLoader.load(
    ...
    function (json) {
      ...
      document.getElementById("nameOfLight").innerHTML =
        getLightName(currentLighting);
...
const lightsUsed = [
  ["pointLight1", "pointLight2", "hemisphereLight"],
...
function generateLighting(num) {
  ...
  if (num === 0) {
    ...
    pointLight1.intensity = 0.8;
    pointLight1.color.setHex(
      getRandomColour(0xcc, 0xfa, 0xc5, 0xfa, 0xcc, 0xfa)
    );
...
function getRandomColour(rL, rU, gL, gU, bL, bU) {
  if (rL === undefined) return ~~(0xffffff * Math.random());
  else {
...
function setBust(num) {
  if (typeof num === "string") num = getBustNumber(num);
...
function getLightName(num, suffix) {
  ...
  if (currentLighting === 0) {
    if (!suffix) return "Three-Quarter";
    else return "Three-Quarter Lighting";
  } else if (currentLighting == 1) {
    if (!suffix) return "Frontal";
    else return "Frontal Lighting";
  } else if (currentLighting == 2) {
    if (!suffix) return "Edge / Rim";
    else return "Edge / Rim Lighting";
  } else if (currentLighting == 3) {
    return "Contre Jour";
  } else if (currentLighting == 4) {
    if (!suffix) return "From Below";
    else return "Light From Below";
  } else if (currentLighting == 5) {
    return "Spotlighting";
  } else if (currentLighting == 6) {
    if (!suffix) return "Multicolour";
    else return "Multicolour Lighting";
  } else return null;
}
...
function setLighting(num) {
  ...
  if (currentLighting !== undefined) {
    var i,
      arr = lightsUsed[currentLighting],
      length = arr.length;
    for (i = 0; i < length; i++) {
...
function render() {
  ...
  if (mesh !== undefined) {
    if (!paused && !beingDragged) mesh.rotation.z += 0.005;
...
function init() {
  /** HTML element init */
  renderer.setSize("600", "380", false);
  renderer.domElement.setAttribute("id", "threeapp");
  document.getElementById("app").appendChild(renderer.domElement);
  canvas = renderer.domElement;
  //$("#app").css("height", ~~($(window).innerWidth()));
  ...
  function mouseDrag(event, ui) {
    ...
    if (!beingDragged) beingDragged = true;
    if (!cameraLocked) {
      camera.position.y = Math.max(
        -10,
        Math.min(savedCameraPosition + yDifference / 20, 10)
      );
